document.addEventListener("DOMContentLoaded", async function () {
    ...
    const productos = ["arroz", "fideos", "azucar", "envioAgregado"];
    
    
    let preciosSuper1 = []; //precios en supermercado 1
    let preciosSuper2 = []; //precios en supermercado 2
    let preciosSuper3 = []; //precios en supermercado 3 
    ...
    async function cargarDatos() {
        fetch('precios.json').then((response) => {
            response.json().then((data) => {
                console.log('Estado del fetch:', response.status);
                console.log('Datos cargados:', data);
                for (var i = 0, len = productos.length; i < len; i++) {
                    preciosSuper1.push(data[productos[i]]["preciosSuper1"]);
                    preciosSuper2.push(data[productos[i]]["preciosSuper2"]);
                    preciosSuper3.push(data[productos[i]]["preciosSuper3"]);
                }
            })
        }).catch((err) => {
